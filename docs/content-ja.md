...
標準 Markdown では raw HTML が必要なことが、lobster.js ではネイティブ記法で書けます。
下の 2 カラムレイアウト？ 純粋な Markdown です。
...
標準 Markdown では raw `<details>` HTML を書く必要があります。lobster.js なら純粋な Markdown です。
...
- [x] 折りたたみブロック
- [x] セル結合テーブル
- [x] シンタックスハイライト（`language-*` クラス出力、ハイライターは自由に選択）
